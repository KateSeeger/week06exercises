...
library(readr)
garden_harvest_raw <- read_csv("https://raw.githubusercontent.com/llendway/scraping_etc/main/2020_harvest.csv", 
...
```{r}
g1 <- garden_harvest %>% 
  filter(vegetable %in% c("spinach", "lettuce", "kale")) %>% 
  gt(
    groupname_col = "vegetable"
  ) %>%
  fmt_date(
    columns = vars(date),
    rows = str_detect(variety, "^[a-d]"),
    date_style = 6
  ) %>%
  tab_header(
    title = "Leafy Greens",
    subtitle = md("Harvested in summer 2020")
  ) %>%
  data_color(columns = vars(weight),
...
g2 <- garden_harvest %>% 
  ...
  tab_header(
    title = "Daily harvest of leafy greens",
  ...
  data_color(columns = vars(cum_harvest_lb),
             colors =  scales::col_numeric(palette = "viridis", 
                                           domain = NULL)) %>%
  tab_options(column_labels.background.color = "coral")
...
graph2 <- garden_harvest %>%
  ...
  labs(title = "Daily harvest of root vegetables",
       x = "", 
       y = "") +
  geom_line()
